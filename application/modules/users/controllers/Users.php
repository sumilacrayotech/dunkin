...
//https://dunkin.demoatcrayotech.com/index.php/users/addUser
    function login()
    {
    ...
    public function customers()
    {
        if($this->ion_auth->logged_in()==1)
        {
            ...
            $crud->add_action('Edit Use',base_url('skin/admin/images/icons/edit.png'),base_url('users/edituser/'), '');
            ...
            $crud->columns('id', 'username','paas_text','storeid','type','email');
    ...
    function test_callback($post_array){
        ...
        $this->input->post('password');        return $post_array;
    }
    ...
    function changeProfileAction()
    {
        ...
        if(!empty($postData['password']) and !empty($postData['current_password'])){
           ...
           // $this->ion_auth->change_password($identity, $postData['current_password'], $postData['password']);
            $this->ion_auth->change_password($identity,$postData['password'], $postData['password']);
            $data = array(
    ...
    function addUser()
    {
        ...
        $this->template->load('admin/advanced_form','addUser',$data);
    }
    ...
    {
        $data['stores'] = $this->store();
        $data['users'] = $this->userdetails($id);
        $this->template->load('admin/advanced_form','editUser',$data);
    }
    ...
    function userdetails($id){
        $this->db->where('id',$id);
        $collection =  $this->db->get('users')->row();
        return $collection;
    }
    ...
    function addAction()
    {
        ...
        $email = $postData['usercode'].'@telldunkinksa.com';
        if ($this->app->check_mail($email) == 0){
            ...
            //$this->ion_auth->register('robert', '123456', 'robert@robert.com', $update_data, array('1') );
        }
    ...
    function editAction(){
        ...
        $this->session->set_flashdata('message_name', '<div style="border: 1px dotted; padding: 5px; color: green; background-color: rgb(221, 221, 221);">User Successfully Edited</div>');
        redirect("users/customers");
