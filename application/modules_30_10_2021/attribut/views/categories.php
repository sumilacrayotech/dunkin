...
<ul class="main_categrory tree" style="float: right; margin: 0px; width: 100% ! important;">
    ...
    foreach ($parent_category as $parent) {
        ...
        <li class="main_cat">
            <?php 
                $check_id = $this->app->check_category($parent->category_id,$this->uri->segment(3));
                if($check_id==1)
                {
                    $checked_id = 'checked=""';
                }
            ...
            <input class="subcat_check nostyle" data-subcat="<?php echo $parent->category_id; ?>" <?php echo $checked_id; ?> type="checkbox"
...
<?php
    ...
    $this->db->where('attribute_id',$this->uri->segment(3));
    $res_att = $this->db->get('attributes')->row();
    $specific_categories = explode(',', $res_att->specific_categories);
    $cate_ids='';
    foreach($specific_categories as $ids)
    {
        $cate_ids .= $this->app->get_main_category($ids);
    }
    $id_array = explode(',',$cate_ids);
    foreach(array_reverse($id_array) as $l => $ides)
    {
        if($ides!=0)
        {
            ?>
            ...
            $(document).ready(function(){
                $.ajax({
                    url: '<?php echo base_url() . 'attribut/sub_cat/'.$this->uri->segment(3);?>',
                    data: 'category_id=<?php echo $ides?>&level=<?php echo $this->app->get_level($ides)?>',
                    type: 'POST',
                    cache:false,
                    success: function (resp) {                       
                        if (resp != '') 
                        {
                            $('#subcat_<?php echo $ides?>').html(resp);
                            $('#subcat_<?php echo $ides?>').show();
                        }
                    }
                });
            });
