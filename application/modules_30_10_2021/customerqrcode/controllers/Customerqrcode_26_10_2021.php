...
class Customerqrcode extends MY_Controller
{
    ...
    function usedVoucher()
    {
        ...
        $crud->columns('id','number', 'name', 'expiry_from','expiry_to','products','custom_products','status','user_code','store');
        $crud->unset_print();
        $output = $crud->render();
    ...
    function activeVoucher()
    {
        ...
        $crud->columns('id','number', 'name', 'expiry_from','expiry_to','products','custom_products','status');
        $crud->unset_print();
        $output = $crud->render();
    ...
    function index(){
        ...
        $crud->columns('id','qr_code','number', 'name', 'expiry_from','expiry_to', 'status');
        ...
        $crud->add_fields(array('name','expiry_from','expiry_to','status','qty'));
        //$crud->edit_fields(array('name','expiry_from','expiry_to','products','company','custom_products','status','email_id','percentage','qr_code'));
         $crud->edit_fields(array('name','expiry_from','expiry_to','status','qr_code'));
        ...
        {
            $finalArray[$row->name]=$row->name;
        }

        if(empty($finalArray)) {
            $companyArray[0] = 'No Company/Customers';
        }else{
            $companyArray = $finalArray;
        }
        $crud->field_type('company','dropdown', $companyArray);
